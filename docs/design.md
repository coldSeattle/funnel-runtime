...
`buildApp().inject`, что позволяет засеять данные при старте (`SEED_TRAFFIC_ON_BOOT=1`).
...
- Детерминированный PRNG по `--seed`; в конце печатает ожидаемые числа (started,
  reachedResult, ctaClicked по вариантам) и сверяет их с `GET /api/analytics`.
